//Collaborated with Joey, Eli, and Dalton
...
PCB* choose_process ()
{
	...
	if(!new_list.empty()){
		...
		if(pid == 0){
				    ...
				    close (nProcess->p2k[READ_END]);
				    close (nProcess->k2p[WRITE_END]);

				    // assign fildes 3 and 4 to the pipe ends in the child
				    dup2 (nProcess->p2k[WRITE_END], 3);
				    dup2 (nProcess->k2p[READ_END], 4);
...
void process_trap (int signum)
{
    ...
    */
	for(int i = 0; i < NUM_PIPES; i += 2){
		cout<<running->k2p[0]<<"    "<<running->k2p[1]<<endl;
		cout<<running->p2k[0]<<"    "<<running->p2k[1]<<endl;
        char buf[1024] ="buf is empty!!!!!!!!!!!";
		
        int num_read = read (running->p2k[READ_END], buf, 1023);
		cout<<"num_read: "<<num_read<<endl;
		cout<<buf<<endl;
		if(num_read > 0){
			const char *msg = "from the kernel to process\n";
			write (running->k2p[WRITE_END], msg, strlen(msg));
		
			
			if(strcmp("requesting process list", buf) == 0){
				char msg2child[1024];
				strcpy(msg2child, "Process list: ");
			
				list<PCB*>::iterator it;
			
				for( it = processes.begin(); it != processes.end(); it++){
					strcat(msg2child, (*it)->name);
					strcat(msg2child, " ");
				}
				write(running->k2p[WRITE_END], msg2child, strlen(msg2child) + 1);
			}

			if (strcmp("requesting system time", buf) == 0){
				cout<<"Child has requested system time"<<endl;
				char msg2child[1024];
				char time[1024];
				cout<<"Sys time is "<< sys_time<<endl;
				strcpy(msg2child, "System time: ");
				sprintf(time, "%d", sys_time);
				cout<<"time:  "<<time<<endl;
				strcat(msg2child, time);
				strcat(msg2child, "\n");
				write(running->k2p[WRITE_END], msg2child, strlen(msg2child) + 1);
			}
...
void create_list(int argc, char** argv){
	...
	for(int i = 1; i < argc; i ++){
		...
		// create the pipes
			...
			assert (pipe (process->p2k) == 0);
			assert (pipe (process->k2p) == 0);

			// make the read end of the kernel pipe non-blocking.
			assert (fcntl (process->p2k[READ_END], F_SETFL,
			fcntl(process->p2k[READ_END], F_GETFL) | O_NONBLOCK) == 0);
